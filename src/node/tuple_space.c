...
static void code_int(char numeric_buffer[static NUMERIC_LENGTH], const int value) {
    for(size_t i = 0;i < NUMERIC_LENGTH;i++) {
        numeric_buffer[i] = value >> (BYTE_LENGTH * (NUMERIC_LENGTH - i));
    }
}
...
static size_t generate_data_segment(const char* tuple_name, field_t fields[const], const int fields_amount) {
    ...
    strncpy(buffer, tuple_name, strlen(tuple_name) + 1);
    // printf("%d:%c\n", (int)buffer[strlen(tuple_name)], (unsigned char)buffer[strlen(tuple_name)]);
        ...
        switch(fields[i].type) {
            case TS_INT: {
                if(fields[i].is_actual == TS_YES) {
                    ...
                    strncat(buffer + data_segment_size, values_buffer, 5);
                    data_segment_size += NUMERIC_LENGTH + 1;
                } else if(fields[i].is_actual == TS_NO) {
                    values_buffer[0] = (char)INT_NO;
                    strncat(buffer + data_segment_size, values_buffer, 1);
                    data_segment_size += 1;
                } else {
            ...
            case TS_STRING: {
                if(fields[i].is_actual == TS_YES) {
                    ...
                    strncat(buffer, values_buffer, 1);
                    strncat(buffer + data_segment_size, fields[i].data.string_field, strlen(fields[i].data.string_field) + 1);   // Adding string to buffer + '\0' character
